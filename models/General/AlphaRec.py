...
class AlphaRec_Data(AbstractData):
    ...
    def add_special_model_attr(self, args):
        ...
        embedding_path_dict = {
            ...
            'lattice_text':  'item_cf_embeds_lattice_text_array.npy',
            'lattice_image': 'item_cf_embeds_lattice_image_array.npy',
            'qwen': 'item_cf_embeds_qwen_array.npy',
            'qwen_text': 'item_cf_embeds_qwen_text_array.npy',
            'qwen_8b': 'item_cf_embeds_qwenvl_8b_array.npy',
            'qwen_8b_text': 'item_cf_embeds_qwen_8b_text_array.npy',
            'qwen_8b_image': 'item_cf_embeds_qwen_8b_image_array.npy',
            'qwentext_8b': 'item_cf_embeds_qwentext_8b_array.npy',
            'qwenvl_8b': 'item_cf_embeds_qwenvl_8b_array.npy',
            'qwenvl_8b_image': 'item_cf_embeds_qwenvl_8b_image_array.npy',
            'qwentext_qwenvl_8b': 'item_cf_embeds_qwentext_qwenvl_8b_array.npy',
            'qwentext_qwenvlimg_8b': 'item_cf_embeds_qwentext_qwenvlimg_8b_array.npy',
        }
        self.item_cf_embeds = np.load(loading_path + embedding_path_dict[self.lm_model])

        # User CF Embedding: the average of interacted item LM embeddings
        # sorted by user_id to match the user index ordering
        uid_to_items: dict[int, list[int]] = {}
        for u, items in self.train_user_list.items():
            uid_to_items[u] = list(items)

        user_cf_embeds_list = []
        for uid in sorted(uid_to_items.keys()):
            item_ids = np.array(uid_to_items[uid])
            user_cf_embeds_list.append(self.item_cf_embeds[item_ids].mean(axis=0))

        self.user_cf_embeds = np.array(user_cf_embeds_list)
...
class AlphaRec(AbstractModel):
    def __init__(self, args, data) -> None:
        ...
        # intermediate dim = multiplier * input_dim ≈ 1152 for all models
        multiplier_dict = {
            'bert': 8,
            'roberta': 8,
            'v2': 2,
            'v3': 1/2,
            'v3_shuffle': 1/2,
            # dim=4096 → intermediate=1152
            'qwenvl_8b':       9/32,
            'qwenvl_8b_image': 9/32,
            'qwentext_8b':     9/32,
            # dim=8192 → 9/64 to keep intermediate=1152 (same as 4096-dim models)
            'qwentext_qwenvlimg_8b': 9/64,
        }
        if(self.lm_model in multiplier_dict):
            multiplier = multiplier_dict[self.lm_model]
        else:
            multiplier = 9/32 # default for dimension = 4096
