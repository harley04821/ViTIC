...
from ....util import DataIterator, typeassert, argmax_top_k, pad_sequences
...
class UniEvaluator(AbstractEvaluator):
    ...
    * **Second**, this class and its evaluation metrics can automatically fit
    ...
    def __init__(self, dataset, user_train_dict, user_test_dict, user_neg_test=None,
                 ...
                 dump_dict=None, masked_items=None):
    ...
    def evaluate(self, model, test_users=None):
        ...
        final_result = np.reshape(final_result, [self.metrics_num, self.max_top])  # (metrics_num, max_top)
        final_result = final_result[:, self.top_show - 1]
        final_result = np.reshape(final_result, [-1])
